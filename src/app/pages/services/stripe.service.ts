   ...
   customerPaymentAttach(paymentMethodId:any, params:any){
    ...
    updateCustomer(customerId: string, updatedData: any): Observable<any> {
      ...
      createSession(params: any) { 
        ...
        payload.set('cancel_url', `https://mockinto-dev.vercel.app/dashboard/cancel-payment`);
...
getCheckoutSession(sessionId: any) {
  ...
  return this.http.post<any>(`${environment.stripeApiUrl}/get-session`, payload)
    .pipe(
      map((data: any) => {
        return data;
      }),
...
createSessionChekout(params: any): Observable<any> {
  return this.http.post<any>(
    `${environment.stripeApiUrl}/checkout`,params ).pipe(map((data: any) => {
      return data;
    }),
    catchError((err) => {
